...
def CNN_Encoder(label1num, images, training,reuse=False):
...
#   cnn_initializer = tf.orthogonal_initializer(gain=0.01)
    ...
    with tf.contrib.framework.arg_scope([layers.conv2d],reuse=reuse,
            ...
            logits = tf.nn.relu(feature)
            logits = slim.fully_connected(logits, num_outputs=label1num, activation_fn=None, scope='sflabel1')
